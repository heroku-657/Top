...
bot_token = '7383314239:AAHFal_jdJ9PAIsx9GxkYSVrkeaVdDwuPyQ'
    ...
    if '{"status":"error","error":{"type":"setup_intent_error","message":"Your card could not be set up for future usage."}}' in confirm_response.text:
        ...
        if bin_data:
            ...
            return (f"**#Stripe_Auth 🌩**\n"
                    ...
                    f"**[[ϟ](t.me/)] Proxy: Live ✅**\n"
                    f"━━━━━━━━━━━━━\n"
                    f"`Dev: @fahimhossen27 👑`", "requires_otp")
    else:
...
async def handle_file(event):
    ...
    if event.file and event.file.name.endswith('.txt'):
        ...
        if len(cards) > 5000:
            await event.respond(f"🚫 You can only check up to 5000 cards at a time. Your file contains {len(cards)} cards.")
            os.remove(file_path)
            return
        ...
        for card in cards:
            ...
            if "Approved" in response:
                ...
                await client.send_message(event.chat_id, response, buttons=[
                    [Button.url("🚀", "https://t.me/fahimhossen27"), Button.url("SUPPORT ✨", "https://t.me/+VssZW7u2USRhMmNl")]
                ], link_preview=False)
            else:
            ...
            buttons = [
                ...
                [Button.inline(f"- Dev. @fahimhossen27 -", b"show_dev")],
            ]
...
async def callback(event):
    ...
    if event.data == b"show_approved":
        ...
        await event.respond("🚀 t.me/fahimhossen27\n\n Join our channelbfor more!") 
